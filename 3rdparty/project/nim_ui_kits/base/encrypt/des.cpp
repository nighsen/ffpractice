...
{
	char* bitsCiphertextAnyLength = new char[data_base_length_*4];
	char* hexCiphertextAnyLength_local = new char[data_base_length_*2];

	int iLen = 0;

	memset(szCiphertextData,0,data_base_length_);
	memset(hexCiphertextAnyLength_local,0,data_base_length_*2);
	

	iLen = (((int)strlen(szCipherInBytes)>>2) + ((int)strlen(szCipherInBytes) % 4 == 0 ? 0 : 1))<<4;
	
	memcpy(hexCiphertextAnyLength_local,szCipherInBytes,strlen(szCipherInBytes));
	Hex2Bits(hexCiphertextAnyLength_local,bitsCiphertextAnyLength,iLen);
	Bits2Bytes(szCiphertextData,bitsCiphertextAnyLength,iLen);

	delete[] bitsCiphertextAnyLength;
	delete[] hexCiphertextAnyLength_local;
